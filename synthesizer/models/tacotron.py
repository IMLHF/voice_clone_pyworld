...
from synthesizer.models.modules import EncoderConvolutions, EncoderRNN, Prenet, FrameProjection
from synthesizer.models.modules import StopProjection, DecoderRNN, Postnet, CBHG, MaskedLinearLoss, MaskedMSE, MaskedSigmoidCrossEntropy
        ...
        for i in range(hp.tacotron_num_gpus):
            with tf.device(tf.train.replica_device_setter(ps_tasks=1, ps_device="/cpu:0",
                ...
                with tf.variable_scope("inference", reuse=tf.AUTO_REUSE):
                    ...
                    # # region add speaker embeding to word embeding
                    # one_hots = tf.reshape(tower_embed_targets[i], [-1, self._hparams.speaker_embedding_size])
                    # # print("___________onehot shape", one_hots.shape.as_list())
                    # with tf.variable_scope('dense_onehot2embeds'):
                    #   dense = tf.layers.dense(one_hots, units=self._hparams.speaker_embedding_size_fin//2, activation='tanh',
                    #                           name="dense_onehot2embeds1")
                    #   embeds = tf.layers.dense(dense, units=self._hparams.speaker_embedding_size_fin, activation=None,
                    #                            name="dense_onehot2embeds2")
                    # tileable_shape = [-1, 1, self._hparams.speaker_embedding_size_fin] # [batch, time, speaker_embedding]
                    # tileable_embed_targets = tf.reshape(embeds, tileable_shape)
                    # tiled_embed_targets = tf.tile(tileable_embed_targets,
                    #                               [1, tf.shape(embedded_inputs)[1], 1])

                    # embedded_inputs = tf.concat((embedded_inputs, tiled_embed_targets), 2)
                    # # endregions

                    # Encoder Cell ==> [batch_size, encoder_steps, encoder_lstm_units]
                    encoder_cell = TacotronEncoderCell(
                        EncoderConvolutions(
                            is_training, hparams=hp, scope="encoder_convolutions"),
                    ...
                    # encoder_cond_outputs = encoder_outputs
