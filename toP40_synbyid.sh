...
if [[ -z "$1" ]]; then
  echo "Default Destination: Real-Time-Voice-Cloning-byid"
  dest="english_voice_clone/Real-Time-Voice-Cloning-byid"
else
  dest="$1"
fi
...
# exit 0
rsync -avh -e "ssh -p 22 -o ProxyCommand='ssh -p 8695 zhangwenbo5@120.92.114.84 -W %h:%p'" --exclude-from='.vscode/exclude.lst' ./* zhangwenbo5@ksai-P40-2:/DATA/disk1/lihongfeng/$dest
